...
def getPkgInfoList():
    ...
    """
    pkgInfoList = []
    logging.info("---Step3.1: get package info")
    pkgPath = parseConf("pkgPath")

    # 防止未取到信息的情况下继续运行
    if pkgPath is None or pkgPath == "":
        logging.error("---Step3.1.1: get package path error; pkgPath not exist")
        raise Exception("fail to get package path; pkgPath not exist")
    logging.info("---Step3.1.1: get package path ok; pkgPath = %s" % pkgPath)
    ...
    # 获取所有包，并记录到日志中
    logging.info("---Step3.1.2: get package list:")
    for root, dirs, files in os.walk(pkgPath):
        for file in files:
            if os.path.splitext(file)[1] == ".tgz":
                logging.info("---Step3.1.2.pkg: %s  %s" % (file, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(os.path.getmtime(root + "\\" + file)))))
...
def drawPkgHTML(html):
    ...
    pkgHtml += "\n     <td colspan=\"2\" align=\"left\"><b><a name=\"gtr\">Package Info</a></b>"
    pkgHtml += "\n </tr>"
    pkgHtml += "\n <tr>"
